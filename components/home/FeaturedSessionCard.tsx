...
export function FeaturedSessionCard({ session }: FeaturedSessionCardProps) {
  ...
  const shellClass =
    "group block overflow-hidden rounded-xl ring-1 ring-stone-900/8 transition hover:ring-stone-900/15";
  ...
  if (!session.href) {
    return (
      <article className={shellClass} aria-label={`${session.title} — coming soon`}>
        {content}
      </article>
    );
  }
